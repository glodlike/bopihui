...
//                laodData();
    ...
    public  void  getAllOrder( ){
        XUtils.Get(AppHttp.BASE_URL_NEW+"ctl=uc_order&status=1&email="+userList.get(0)+"&pwd="+userList.get(1)+"",null,new MyCallBack<String>(){
            @Override
    ...
    public  void  getMoreData(int pages){
        XUtils.Get(AppHttp.BASE_URL_NEW+"ctl=uc_order&status=1&page="+pages+"&email="+userList.get(0)+"&pwd="+userList.get(1)+"",null,new MyCallBack<String>(){
            @Override
            public void onSuccess(String result) {
                OrderListInfo orderListInfo = new Gson().fromJson(StringUtils.base64ToString(result),OrderListInfo.class);
