 ...
 */
public class UnDeliverOrderFragment extends BaseFragment {
...
//                laodData();
    ...
    public  void  getAllOrder( ){
        XUtils.Get(AppHttp.BASE_URL_NEW+"ctl=uc_order&status=2&email="+userList.get(0)+"&pwd="+userList.get(1)+"&city_id="+new SharePerfenceUtils(getContext()).getShop_id()[0]+"",null,new MyCallBack<String>(){
            @Override
    ...
    public  void  getMoreData(int pages){
        XUtils.Get(AppHttp.BASE_URL_NEW+"ctl=uc_order&status=2&page="+pages+"&email="+userList.get(0)+"&pwd="+userList.get(1)+"&shop_id="+new SharePerfenceUtils(getContext()).getShop_id()[0]+"",null,new MyCallBack<String>(){
            @Override
            public void onSuccess(String result) {
                OrderListInfo orderListInfo = new Gson().fromJson(StringUtils.base64ToString(result),OrderListInfo.class);
